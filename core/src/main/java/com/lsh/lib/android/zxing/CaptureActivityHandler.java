...
public class CaptureActivityHandler extends Handler {
    ...
    public CaptureActivityHandler(Object obj,
        ...
        if (obj instanceof CaptureActivity) {
            ...
            decodeThread = new DecodeThread(obj, decodeFormats, baseHints,
                    characterSet, new ViewfinderResultPointCallback(
                    ((CaptureActivity) obj).getViewfinderView()));
        } else if (obj instanceof ZxingFragment) {
            decodeThread = new DecodeThread(obj, decodeFormats, baseHints,
                    characterSet, new ViewfinderResultPointCallback(
                    ((ZxingFragment) obj).getViewfinderView()));
        }

        decodeThread.start();
        state = State.SUCCESS;
        if (obj instanceof CaptureActivity) {
            activity = (CaptureActivity) obj;
        } else if (obj instanceof ZxingFragment) {
            activity = ((ZxingFragment) obj).getActivity();
        }
    ...
    public void handleMessage(Message message) {
        ...
        if (message.what == R.id.restart_preview) {
            ...
            if (obj instanceof CaptureActivity) {
                ((CaptureActivity) obj).handleDecode((Result) message.obj, barcode, scaleFactor);
            } else if (obj instanceof ZxingFragment) {
                ((ZxingFragment) obj).handleDecode((Result) message.obj, barcode, scaleFactor);
            }
    ...
    private void restartPreviewAndDecode() {
        if (state == State.SUCCESS) {
            ...
            cameraManager.requestPreviewFrame(decodeThread.getHandler(),
                    R.id.decode);
            if (obj instanceof CaptureActivity) {
                ((CaptureActivity) obj).drawViewfinder();
            } else if (obj instanceof ZxingFragment) {
                ((ZxingFragment) obj).drawViewfinder();
            }
        }
    }
